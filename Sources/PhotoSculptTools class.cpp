 ...
 *
 *  Created on: 23 avril 2009
  *      Author: Hippolyte Mounier, Copyright 2010 All rights reservedlyte Mounier
 *              Copyright 2010, 
 *              All rights reserved
 */
                              ...
                              float strength, float midlevel)
{
  ...
  if (sizex!= map.getSizeX() || sizey!= map.getSizeY())
    THROW_INTERNAL_ERROR("Incorrect parameter, sizes of image and map are different (in applyDisplace())");
...
class ComputeEdgeDetection : public Parallelable
{
private:
  const Image* image_  ;
  PFloatMap    result_ ;
...
public:
  ComputeEdgeDetection(const Image* image,  PFloatMap result, int sizex,  int sizey):
    image_ ( image ), result_ ( result ), sizeX_ ( sizex ), sizeY_ ( sizey )
  {  }
  ...
  void task(int y)
  {
      for (int x=0; x<sizeX_; x++)
      {
        ...
        ColorF center = getPixelF(*image_,x,y);
        ...
        for ( int b = 0 ; b < GlobalBurstMapUsage ; b++ )
        {
          ...
          if (0<=burstX && burstX<sizeX_ && 0<=burstY && burstY<sizeY_)
          {
            ColorF side = getPixelF(*image_,burstX,burstY);
        ...
        result_->set(x,y, 3.0f-mostDifferentColorValue );
...
{
  DEBUG(computeEdgeDetection);
  // Reads image size, to create a new one same size
  int sizex = image.dimx();
  int sizey = image.dimy();

  if (sizex == 0 || sizey == 0)
    THROW_INTERNAL_ERROR("Empty input image (in computeEdgeDetection())");

  int BurstMapUsage=BurstMap::getMaxTableIndex(range);

  PFloatMap result = new FloatMap(sizex,sizey); // initiates a default empty mask
  ...
  ComputeEdgeDetection computer(&image, result, sizex, sizey);
  computer.launchTask(sizey);
...
void smartBlurLine(int y, void * unused)
{
...
{
  DEBUG(smartBlur);
  if (leftImage->dimx() != map->getSizeX() || leftImage->dimy() != map->getSizeY() )
    THROW_INTERNAL_ERROR( "Wrong parameter, image size and map size are different (in smartBlur() )");
  ...
  //Image trial=smartBlur(leftImage,threshold,range);
  //trial.save("This_is_a_smart_blur_test.bmp");
...
void blurMaskedLine(int y, void * unused)
{
  for (int x=0;x<GlobalSizeX;x++)
  {
...
class ComputeBlurMap : public Parallelable
{
private:
  int              sizeX_, sizeY_, burstMapUsage_ ;
  VectorMap        returned_      ;
  const VectorMap* map_           ;
   
public:
  ComputeBlurMap(int range, const VectorMap* map): map_(map)
  { 
    sizeX_=map_->getSizeX();
    sizeY_=map_->getSizeY();

    // creates the new map
    returned_ = VectorMap(sizeX_,sizeY_);

    // gets the exact number of pixel to use for blurring 1 pixel
    burstMapUsage_ = BurstMap::getMaxTableIndex(range);
  }
  ...
  void task(int y)
  {
    for (int x=0; x<sizeX_; x++)
    {
      ...
      for (int b=0; b<burstMapUsage_; b++)
      {
        UserInterrupt::stopIfOrdered();
        ...
        if (xx>=0 && xx<sizeX_ && yy>=0 && yy<sizeY_)
        {
          Vector d=map_->get(xx,yy);
          nb+=1;
          u+=d.x();
          v+=d.y();
        }
...
VectorMap PhotoSculptTools::blurMap(const VectorMap& map, int range)
{
  ...
  ComputeBlurMap computer(range, &map);
  computer.launchTask(map.getSizeY());
...
Image PhotoSculptTools::generateMorphed(const Image& right, const VectorMap & map, bool extend)
{
  ...
  if (right.dimx() != sizex || right.dimy() != sizey)
    THROW_INTERNAL_ERROR("Wrong arguments, image and map are different (in generateMorphed())");
...
Image PhotoSculptTools::generateMorphedLeft(const Image& left, const VectorMap & map, bool extend)
{
  ...
  if (left.dimx() != sizex || left.dimy() != sizey)
    THROW_INTERNAL_ERROR("Wrong arguments, image and map are different (in generateMorphedLeft())");
...
void PhotoSculptTools::mixByAlpha(PIntMap original, const PIntMap modification, const PImage alpha)
{
  ...
  if (modification->getSizeX() != sizex || modification->getSizeY() != sizey)
    THROW_INTERNAL_ERROR("Wrong arguments, modification and original are different in size (in mixByAlpha())");
